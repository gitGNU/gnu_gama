...
/*
 *  $Id: encoding.cpp,v 1.4 2005/08/21 20:36:01 cepek Exp $
 */
...
int cp1251_unicode(int* tab){
   int cp1251_unicode[]={
         0x00000080,   /* 0x80 */      // NOW only 1250 for debugging !!!!!!! 
         0x00000081,   /* 0x81  ? */
         0x0000201A,   /* 0x82 */
         0x00000083,   /* 0x83  ? */
         0x0000201E,   /* 0x84 */
...
char* utf8_cp1251(char *buf){      // NOW only 1250 for debugging !!!!!!! 
  static int tab[256];
  // static int itab=cp1251_unicode((int*)tab);  ... rewritten to avoid warning
  static bool init_tab = true;
  if (init_tab)  
    {       
...
int UnknownEncodingHandler(void *userData, const char *name,XML_Encoding *info)
{
 ...
 else if (!strcmp(name,"cp-1251")) cp1251_unicode(info->map);
 else if (!strcmp(name,"windows-1251")) cp1251_unicode(info->map); /* this is probably correct */
